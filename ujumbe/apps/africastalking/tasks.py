...
def end_user_subscription(phonumber: str, subscription_id: int):
    ...
    s.deactivate()
    send_sms.delay(phonenumber=phonumber,
...
def set_user_location(location_id: int, phonenumber: str):
    ...
    text = ""
    init_loc_text = "Your location has been changed from {}".format(
        str(p.location)) if p.location else None
    p.location = l
    p.save()
...
def check_and_send_user_subscriptions():
    ...
    if subscriptions.count() > 0:
        for subscription in subscriptions:
            try:
                send_sms.delay(text=subscription.sms_description,
