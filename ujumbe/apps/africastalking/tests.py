...
class MessageProcessingTests(APITestCase):
    @patch("ujumbe.apps.africastalking.tasks.send_bulk_sms")
    def test_same_shortcode_phonenumber(self, mock_send_sms):
        ...
        print("test_same_shortcode_phonenumber {}".format(responses))
    ...
    def test_returned_response(self, mock_send_sms):
        ...
        self.assertEqual(responses[0]["text"], "Hello Jerry Shikanga, your account has been created successfully. Your "
                                               "username is jerryshikanga")

    @patch("ujumbe.apps.africastalking.tasks.send_bulk_sms")
    def test_invalid_keyword(self, mock_send_sms):
    ...
    @patch("ujumbe.apps.africastalking.tasks.send_bulk_sms")
    def test_unregistered_user_response(self, mock_send_sms):
        G(IncomingMessage, processed=False, text="Subscribe", phonenumber="+254739936705")
        mock_send_sms.return_value = None
        responses = process_incoming_messages()
        self.assertEqual(str(responses[0]["text"]), "Please create an account by sending REGISTER*{first_name}*{"
                                                    "last_name}.")

    @patch("ujumbe.apps.africastalking.tasks.send_bulk_sms")
    def test_reregister(self, mock_send_sms):
